	...
	setUpTerritoriesObj: function() {
		for(id in TerritoryNames) {
			...
			var territoryNameImg = new Kinetic.Image({
				image: sprite,
				x: 0, //FontDestinationCoords[id].x,
				y: 0, //FontDestinationCoords[id].y,
				width: 0, //FontSpriteCoords[id].sWidth, //'destiantion Width' 
				height: 0, //FontSpriteCoords[id].sHeight, //'destination Height'
				crop: [0,0,0,0]//[FontSpriteCoords[id].sx, FontSpriteCoords[id].sy, FontSpriteCoords[id].sWidth, FontSpriteCoords[id].sHeight]
		...
		for (t in Risk.Territories) {
			...
			(function(path, t, group) {
				...
				group.on('click', function() {
					...
					var url = "http://enterbt.hu/dev/psafe/irodak?field_megye_value=" + path.attrs.id;
